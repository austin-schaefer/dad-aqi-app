  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {cities.map((city, index) => {
        ...
        // Latest AQI: last non-null entry
        let latestAqi: number | null = null;
        if (data) {
          for (let i = data.entries.length - 1; i >= 0; i--) {
            if (data.entries[i]!.aqi !== null) {
              latestAqi = data.entries[i]!.aqi;
              break;
            }
          }
        }
        ...
        return (
          ...
          >
            ...
            <div
              className={[
                'w-3 h-3 rounded-full shrink-0 transition-shadow duration-150',
                isHidden ? '' : 'shadow-[0_0_6px_var(--swatch-color)]',
              ].join(' ')}
              style={{ backgroundColor: color, '--swatch-color': color } as React.CSSProperties}
            />

            {/* City name — clicking toggles visibility */}
            <button
            ...
            >
              <div className={[
                ...
                isPendingRemove ? 'text-red-300' : isHidden ? 'text-slate-500 line-through' : 'text-slate-200',
              ].join(' ')}>
                {city.name}
              </div>
              {city.admin1 && (
                <div className={[
                  'text-xs truncate',
                  isPendingRemove ? 'text-red-400/60' : 'text-slate-500',
                ].join(' ')}>{city.admin1}</div>
              )}
            </button>

            {/* AQI value / status — fixed width to prevent squeezing city name */}
            {!isPendingRemove && (
                ...
                {isLoading ? (
                  ...
                  <div>
                    <div
                      ...
                      style={{ color: isHidden ? '#64748b' : (category?.color ?? '#94a3b8') }}
                    >
                      {latestAqi}
                    </div>
                    {category && (
      ...
      {cities.length === 0 && (
        <div className="text-xs text-slate-500 text-center py-4">
          No cities added. Search above to add one.
        </div>
      )}
    </div>
  );
